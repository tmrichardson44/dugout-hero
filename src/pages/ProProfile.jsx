  ...
  return (
    <div className="min-h-screen bg-white/40 ">
      <nav className="bg-blue-600 text-white p-4 flex items-center gap-4 shadow-lg sticky top-0 z-50">
        ...
        <div className="font-bold tracking-widest uppercase text-sm">Profile Settings</div>
      </nav>
      ...
      <main className="max-w-2xl mx-auto p-4 sm:p-8 space-y-8">
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 hover:-translate-y-1 hover:shadow-md transition-all duration-300">
           <div className="flex items-center gap-6 mb-6">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-800 tracking-tighter uppercase">{currentUser?.displayName || 'Lineup Hero User'}</h1>
                <p className="text-slate-500 font-bold">{currentUser?.email}</p>
                {currentUser?.systemRole === 'super_admin' && <span className="inline-block mt-2 bg-rose-100 text-rose-700 text-[10px] uppercase font-bold tracking-widest px-2 py-1 rounded-md">Super Admin</span>}
                {currentUser?.systemRole === 'user' && <span className="inline-block mt-2 bg-emerald-100 text-emerald-700 text-[10px] uppercase font-bold tracking-widest px-2 py-1 rounded-md">Pro User</span>}
              </div>
           </div>
        </section>
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 hover:-translate-y-1 hover:shadow-md transition-all duration-300 space-y-6">
          ...
          {teams.length === 0 ? (
            <div className="text-slate-400 text-sm font-bold bg-white/40  p-6 rounded-lg text-center">
               You are not currently managing any teams.
            </div>
          ) : (
            <div className="space-y-3">
              {teams.map(team => (
                <div key={team.id} className="bg-white/40  p-4 rounded-lg justify-between flex items-center border border-slate-100">
                  <div>
                    <h3 className="font-bold text-slate-800 uppercase">{team.name}</h3>
                    <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">{team.program || 'Independent'}</p>
                  </div>
                  <Link to={`/pro/team/${team.id}`} className="text-xs bg-white border border-slate-200  text-emerald-600 font-bold tracking-wide px-4 py-2 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 hover:-translate-y-1 hover:shadow-md transition-all duration-300 hover:border-emerald-500 transition-colors">
                    Manage
                  </Link>
                </div>
              ))}
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 hover:-translate-y-1 hover:shadow-md transition-all duration-300 space-y-6">
          ...
          <form onSubmit={handleUpdateName} className="space-y-4">
            ...
            {nameSuccess && <div className="bg-emerald-50 text-emerald-600 font-bold text-sm p-4 rounded-xl">{nameSuccess}</div>}

            <div>
              <label className="block text-[10px] font-bold text-slate-400 tracking-wide font-medium ml-1 mb-2">Display Name</label>
              <input 
                type="text" 
                required 
            ...
            <button 
              ...
              className="mt-2 bg-blue-600 text-white font-bold text-xs tracking-wide font-medium py-3 px-6 rounded-lg shadow-lg shadow-blue-200 hover:bg-blue-700 transition flex items-center gap-2 active:scale-95 disabled:opacity-50"
            >
              Update Name
            </button>
          </form>
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 hover:-translate-y-1 hover:shadow-md transition-all duration-300 space-y-6">
          ...
          <form onSubmit={handleUpdatePassword} className="space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 tracking-wide font-medium ml-1 mb-2">New Password</label>
              <input 
                type="password" 
                required 
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 tracking-wide font-medium ml-1 mb-2">Confirm New Password</label>
              <input 
                type="password" 
                required 
                className="w-full bg-white border border-slate-200 rounded-lg px-5 py-3 font-bold text-slate-700 outline-none focus:bg-white border border-slate-200 focus:border-blue-500 transition-all"
        ...
        {/* Danger Zone */}
        <section className="bg-rose-50 rounded-xl p-8 shadow-inner border-2 border-rose-100 space-y-6 mt-12">
           <div className="flex items-center gap-3 border-b border-rose-200 pb-4">
             <Trash2 className="text-rose-500 w-6 h-6" />
             <h2 className="text-lg font-bold text-rose-800 tracking-wide font-medium">Danger Zone</h2>
           </div>
           <div>
             ...
             <button 
                ...
                disabled={isDeleting}
                className="bg-rose-600 text-white font-bold text-xs tracking-wide font-medium py-3 px-6 rounded-lg shadow-lg shadow-rose-200 hover:bg-rose-700 transition active:scale-95 disabled:opacity-50"
             >
                Delete Account
             </button>
           </div>
