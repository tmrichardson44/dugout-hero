...
export default function ProTeamDashboard() {
    ...
    const fetchTeam = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          let hasAccess = false;
          if (isSuperAdmin() || data.managerUid === currentUser.uid) {
            hasAccess = true;
          } else {
          ...
          if(data.seasonSettings) setSeasonConfig({...data.seasonSettings, program: data.program, leagueId: data.leagueId || '', season: data.season || 'Spring', year: data.year || '2026'});
          else setSeasonConfig({ ...INITIAL_CONFIG, teamName: data.name, program: data.program, leagueId: data.leagueId || '', season: data.season || 'Spring', year: data.year || '2026' });
        } else {
  ...
  const handleUpdateSeasonConfig = async (newConfig) => {
    const { program, leagueId, season, year, ...restConfig } = newConfig;
  ...
  return (
    <DugoutHeroCore
      ...
      canDeleteTeam={teamDoc?._isLeagueAdminTrue || isSuperAdmin()}
