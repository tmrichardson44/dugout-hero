  ...
  return (
    <div className="bg-white/60 backdrop-blur-xl border border-white/40  rounded-[28px] border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(16,185,129,0.1)] transition-all duration-300 overflow-hidden group hover:shadow-lg hover:border-violet-200 transition-all">
      <div className="p-6 pb-4">
        ...
        {hasCoach ? (
          <div className="flex items-center gap-2 bg-white/40 backdrop-blur-sm border border-slate-100 px-3 py-1.5 rounded-xl mb-3">
            <ShieldCheck className="w-3 h-3 text-slate-500 shrink-0" />
            <span className="text-[10px] font-black text-slate-600 tracking-wide font-medium truncate">{displayCoachName}</span>
          </div>
        ) : (
          <div className="flex items-center gap-2 bg-white/40 backdrop-blur-sm border border-slate-100 px-3 py-1.5 rounded-xl mb-3 opacity-60">
            <Mail className="w-3 h-3 text-slate-400 shrink-0" />
            <span className="text-[10px] font-black text-slate-400 tracking-wide font-medium">No coach assigned</span>
          </div>
        )}

        <div className="space-y-1.5">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-black text-slate-400 tracking-wide font-medium">Team Avg</span>
      ...
      {showCoachModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={() => setShowCoachModal(false)}>
          <div className="bg-white/60 backdrop-blur-xl border border-white/40  rounded-[28px] p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <select
              className="w-full bg-white/40 backdrop-blur-sm border border-slate-200 rounded-2xl px-5 py-3 font-bold text-slate-700 outline-none focus:border-slate-500 transition-all mb-4"
            ...
            {assignCoachMode === 'new' && (
              <>
                ...
                <input
                  ...
                  className="w-full bg-white/40 backdrop-blur-sm border border-slate-200 rounded-2xl px-5 py-3 font-bold text-slate-700 outline-none focus:border-slate-500 transition-all mb-2"
      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white/40  rounded-3xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="p-8 pb-4 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-[10px] font-black text-slate-400 tracking-wide font-medium mt-1">Target: {low.toFixed(1)}–{high.toFixed(1)}</p>
          </div>
...
export default function ProDivisionDashboard() {
  ...
  if (loading || !division) {
    return <div className="min-h-screen bg-white/40 backdrop-blur-sm flex items-center justify-center"><div className="w-8 h-8 border-4 border-slate-500 border-t-transparent rounded-full animate-spin" /></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-white/40 backdrop-blur-sm">
      <nav className="bg-slate-600 text-white p-4 flex items-center justify-between shadow-lg sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <Link to="/pro/dashboard" className="p-2 hover:bg-slate-700 rounded-xl transition-colors text-slate-200 hover:text-white">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <div className="font-black tracking-widest uppercase text-sm leading-none">{division.name}</div>
            <div className="text-slate-300 text-[10px] font-bold tracking-wide font-medium mt-0.5">Division Admin Dashboard</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowAddTeam(true)}
            className="bg-slate-700 hover:bg-slate-800 text-white text-[10px] font-black px-4 py-2 rounded-xl transition-colors flex items-center gap-2 tracking-wide font-medium"
          >
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-8">
        ...
        {teams.length > 0 && (
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4 border-t border-slate-200 pt-8">
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Division Teams</h2>
            
            <div className="flex gap-2">
              <select value={filterSeason} onChange={e => setFilterSeason(e.target.value)} className="bg-white/60 backdrop-blur-xl border border-white/40  border border-slate-200 rounded-xl px-3 py-2 text-xs font-black text-slate-600 tracking-wide font-medium outline-none focus:border-slate-500 appearance-none cursor-pointer">
              ...
              <select value={filterYear} onChange={e => setFilterYear(e.target.value)} className="bg-white/60 backdrop-blur-xl border border-white/40  border border-slate-200 rounded-xl px-3 py-2 text-xs font-black text-slate-600 tracking-wide font-medium outline-none focus:border-slate-500 appearance-none cursor-pointer">
        ...
        {teams.length === 0 ? (
          <div className="bg-white/60 backdrop-blur-xl border border-white/40  rounded-3xl p-16 flex flex-col items-center justify-center border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(16,185,129,0.1)] transition-all duration-300 mt-8">
            ...
            <button onClick={() => setShowAddTeam(true)} className="bg-slate-100 text-slate-700 px-6 py-3 rounded-2xl font-black text-sm tracking-wide font-medium hover:bg-slate-200 transition flex items-center gap-2">
          ...
          <div className="bg-white/40 backdrop-blur-sm rounded-3xl p-16 flex flex-col items-center justify-center border border-slate-200 border-dashed">
            ...
            <button onClick={() => { setFilterSeason('All Seasons'); setFilterYear('All Years'); }} className="text-slate-600 font-black text-xs tracking-wide font-medium hover:underline">Clear Filters</button>
          </div>
        ) : (
      ...
      {showAddTeam && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => setShowAddTeam(false)}>
          <div className="bg-white/60 backdrop-blur-xl border border-white/40  rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Add Team to Division</h3>
            ...
            <input
              className="w-full bg-white/40 backdrop-blur-sm border border-slate-200 rounded-2xl px-5 py-3 font-bold text-slate-700 outline-none focus:border-slate-500 transition-all mb-4 text-sm"
            ...
            <div className="space-y-2 max-h-52 overflow-y-auto mb-4">
              {filteredTeams.length === 0 && (
                <p className="text-slate-400 font-bold text-sm text-center py-4">No available teams found in this league.</p>
              )}
              {filteredTeams.map(team => (
                <button
                  ...
                  disabled={addingTeam}
                  className="w-full flex items-center justify-between bg-white/40 backdrop-blur-sm hover:bg-white/40 backdrop-blur-sm hover:border-slate-300 border border-slate-200 p-4 rounded-2xl transition-all text-left"
                >
            ...
            <p className="text-xs text-slate-400 text-center font-bold">Only teams already in this division's parent league can be added.</p>
          </div>
        </div>
      )}
